...
//       {items.map((item) => {
//         // Parse the image URL from JSON array
//         let imageUrl = '/placeholder.svg';
//         if (item.product?.image_url) {
//           try {
//             const images = JSON.parse(item.product.image_url);
...
//             }
//           } catch {
//             // If parsing fails, try using it as direct URL
...
export default function Checkout() {
  ...
  const handlePlaceOrder = async () => {
    ...
    const orderNumber = `ORD-${dateStr}-${Math.floor(Math.random() * 1000000).toString().padStart(6, '0')}`;
